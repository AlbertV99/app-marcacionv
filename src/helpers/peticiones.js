...
const Peticiones = () => {
    ...
    const obtenerPanel = async (
        ...
        const url =
            BASE + modulo + "/" + pagina + "/" + (buscar !== "" ? buscar : "");
    ...
    const procesoDeEnvio = async (datos) => {
        ...
        try {
            ...
            if (e.message.includes("11")) {
                resp = {
                    cod: "11",
                    msg: "No pueden registrarse 2 marcaciones offline de la misma persona",
                };
            } else if (e.message.includes("09")) {
                resp = {
                    cod: "09",
                    msg: "Tipo de marcacion ya registrada para este personal",
                };
            } else if (e.message.includes("12")) {
                resp = { cod: "12", msg: "Llego al maximo de offline" };
            } else if (e.message.includes("15")) {
                resp = { cod: "15", msg: "Error en consistencia de datos" };
            } else {
                resp = {
                    cod: "99",
                    msg: "No se pudo realizar el envio de datos",
                };
            }
        } finally {
        }
    ...
    const actualizacionListaEnviar = async (datos) => {
        ...
        let validaciones = {
            marca: await verificarMarca(
                datos.personal_id,
                datos.tipo_marcacion,
            ),
            lista: verificarLista(datos.personal_id),
        };
    ...
    const actualizacionUltimaUbicacion = (lat, lon) => {
        ...
        if (lat != "0" && lon != "0") {
            localStorage.setItem(
                "ubicacion",
                JSON.stringify({ latitud: lat, longitud: lon }),
            );
        }
    };
    ...
    const obtenerUltimaUbicacion = () => {
        return (
            JSON.parse(localStorage.getItem("ubicacion")) || {
                latitud: 0,
                longitud: 0,
            }
        );
    ...
    const eliminarPosListaEnviar = (pos) => {
        const lista = JSON.parse(
            localStorage.getItem("listaRegistros") || "[]",
        );
        lista.splice(pos);
    ...
    const obtenerPersonales = async () => {
        const query =
            "SELECT id,nombres,apellidos,nro_docum,dsc_cargo FROM personal ;";
    ...
    const verificarMarca = async (id, marca) => {
        ...
        let elegido = pers.find(
            (elemento) => elemento.id == id && elemento.marca == marca,
        );
        console.log(elegido, "marca", id, marca);
        if (typeof elegido != "undefined") {
            // SI LA MARCA INTENTANDO DE REGISTRAR ES IGUAL A LA QUE ULTIMO SE MARCO
    ...
    const verificarLista = (id) => {
        ...
        if (typeof existe != "undefined") {
            // SI EXISTE EL ID EN LA LISTA A ENVIAR EN UN FUTURO
            return false; // RETORNO PARA ERROR
        } else {
            return true; // RETORNO SATISFACTORIO
        }
